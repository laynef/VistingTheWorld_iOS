...
extension DetailViewController {
    ...
    private func displayImageFromFlickrByLocation(methodParameters: [String:AnyObject], withPageNumber: Int) {
        ...
        let task = session.dataTaskWithRequest(request) { (data, response, error) in
            ...
            if photoArray.count == 0 {
                ...
                if let imageData = NSData(contentsOfURL: imageUrl!) { // performUIUpdatesOnMain() {
                      cell.collectionCellImageView.image = UIImage(data: imageData)
                    let currentPhoto: Photo = Photo(title: photoTitle, imagePath: imageUrlString, location: nil)
                    // }
                } else {
